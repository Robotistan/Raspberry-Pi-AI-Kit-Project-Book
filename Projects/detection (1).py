...
def send_value(value):
    # Specify the IP address and port of the Raspberry Pi Pico W
    host = '10.0.1.142'  # Pico W IP address...
    port = 12345

    # Create a socket and connect to the server
    with socket.socket(socket.AF_INET, socket.SOCK_STREAM) as s:
        s.connect((host, port))
        s.sendall(str(value).encode())  # Send the numeric value as a string
...
def app_callback(pad, info, user_data):
    ...
    for detection in detections:
        ...
        """if label == "scissors":
            string_to_print += f"Scissors: {label} {confidence:.2f}\n"
            scissors_count += 1
            if confidence > 0.70:
                send_value(28)"""
    ...
    if user_data.use_frame:
        ...
        #cv2.putText(frame, f"Scissors: {scissors_count}", (10, 90), cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 255, 0), 2)
